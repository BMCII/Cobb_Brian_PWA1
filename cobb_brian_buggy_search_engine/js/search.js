...
(function(){                                                                                //create function
	...
	var validqte == function(query){                                                        //create function query
		
		// Trim whitespace from start and end of search query
		while(query.charAt(0) = " "){                                                       //while loop for function
			query = query.substring(1, query.length);
		};
		while(query.charAt(query.length-1) === ""){                                         //second while loop in function
	...
	var search = function(query)                                                            //create search function
		...
		var queryArray = query.join(" ");                                                   //change  search string to an array
	...
	var showMatches = function(results){                                                    //create function showMatches
		...
		for(var i=0, j=results.length; i<j; i++){                                           //for loop through results
			...
			titleEnd = results[i].indexOf('|');                                             //declare variable titleEnd
